...
# %%
'''
FONTOS: Az első feladatáltal visszaadott 
DataFrame-et kell használni a további feladatokhoz. 
A függvényeken belül mindig készíts 
egy másolatot a bemenő df-ről, (new_df = df.copy() és ezzel dolgozz tovább.)
'''
...
def dict_to_dataframe(test_dict:dict) -> pd.DataFrame:
    ...
    return test_df
#asd = dict_to_dataframe(stats)


# %%
'''
Készíts egy függvényt ami a bemeneti DataFrame-ből vissza 
adja csak azt az oszlopot amelynek a neve a bemeneti string-el megegyező.
...
# %%
def get_column(test_df:pd.DataFrame, area:str) -> pd.DataFrame:
    column = test_df[area]
    return column
...
vissza adja a két legnagyobb területű országhoz tartozó sorokat.
...
def get_top_two(test_df:pd.DataFrame):
    newDf = test_df.copy()
    newDf = newDf.sort_values(by="area", ascending = False)
    newDf = newDf.head(2)
    return newDf
...
#asd2 = population_density(asd)

# %%
'''
Készíts egy függvényt, ami a bemeneti Dataframe adatai 
alapján elkészít egy olyan oszlopdiagramot (bar plot),
